...
router.get('/wordpress-plugin-download', authenticateAdsiteToken, async (req, res) => {
  try {
    ...
    // Verificar se o token é válido e ativo
    const adsite = req.adsite; // Vem do middleware authenticateAdsiteToken
    if (!adsite || adsite.status !== 'active') {
      return res.status(403).json({ 
        error: 'Token inválido ou AdSite inativo',
        details: 'Apenas AdSites ativos podem baixar o plugin'
      });
    }

    console.log(`📦 Plugin download requested by AdSite: ${adsite.name} (ID: ${adsite.id})`);
    ...
    try {
      // Main plugin file
      archive.file(path.join(pluginDir, 'url-shortener-adsite.php'), { name: 'url-shortener-adsite/url-shortener-adsite.php' });
      
      // Include directory
      const includesDir = path.join(pluginDir, 'includes');
      if (fs.existsSync(includesDir)) {
        archive.directory(includesDir, 'url-shortener-adsite/includes');
      }
      
      // Assets directory
      const assetsDir = path.join(pluginDir, 'assets');
      if (fs.existsSync(assetsDir)) {
        archive.directory(assetsDir, 'url-shortener-adsite/assets');
      }
      
      // README if exists
      const readmePath = path.join(pluginDir, 'README.md');
      if (fs.existsSync(readmePath)) {
        archive.file(readmePath, { name: 'url-shortener-adsite/README.md' });
      }
      
    } catch (fileError) {
      console.error('🚨 Error adding files to archive:', fileError);
      return res.status(500).json({ error: 'Error creating plugin archive' });
    }
    
    // Finalize the archive
    await archive.finalize();
    
    console.log(`✅ Plugin downloaded successfully by ${adsite.name}`);
